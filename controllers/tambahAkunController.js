...
exports.tambahAkun = async (req, res) => {
  ...
  const query = 'INSERT INTO pengguna (email, nama, password, role, no_telepon, alamat) VALUES (?, ?, ?, ?, ?, ?)';
  db.query(query, [email, nama, hashedPassword, role, no_telepon, alamat], (err, result) => {
    if (err) {
      console.error(err);
